...
설명지금 "상담원 관점" 섹션이 쓰는 BigQuery 쿼리 두 개(상담원 단위,
상담 단위)를 그대로 실행한 결과를, data/agents_snapshot.csv와
data/agent_consultations_snapshot.csv로 각각 저장하는 스크립트를
만들어서 실행해줘. 한글이 깨지지 않게 utf-8-sig로 저장해줘