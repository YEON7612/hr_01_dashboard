# -*- coding: utf-8 -*-
"""
부서별 퇴사인원(막대, 왼쪽 축) x 퇴사율(꺾은선, 오른쪽 축) 결합 차트 (dual-axis)
...
# ------------------------------------------------------------------
# 2. 사번 기준으로 직원-퇴사이력 연결 -> 부서별 퇴사인원, 퇴사율 계산
# ------------------------------------------------------------------
merged = employee.merge(resign[["사번", "퇴사일", "퇴사사유"]], on="사번", how="left")
...
# 3. 퇴사인원 낮은 순으로 정렬
# ------------------------------------------------------------------
dept_summary = dept_rate.sort_values("퇴사인원", ascending=True).reset_index(drop=True)

print("=== 부서별 퇴사인원 x 퇴사율 (퇴사인원 낮은 순) ===")
print(dept_summary.to_string(index=False))

# ------------------------------------------------------------------
# 4. plotly 결합 차트 (dual-axis: 막대=퇴사인원, 꺾은선=퇴사율)
# ------------------------------------------------------------------
...
fig.add_trace(
    go.Bar(
        ...
        name="퇴사인원",
        marker_color="#4C72B0",
        text=dept_summary["퇴사인원"].apply(lambda v: f"{v}명"),
        textposition="outside",
        hovertemplate="<b>%{x}</b><br>퇴사인원: %{y}명<extra></extra>",
    ),
    secondary_y=False,
)
...
fig.update_layout(
    title="부서별 퇴사인원 x 퇴사율 (퇴사인원 낮은 순 정렬)",
...
fig.update_yaxes(title_text="퇴사인원(명)", secondary_y=False,
                  range=[0, dept_summary["퇴사인원"].max() + 3])
...
html_path = os.path.join(OUTPUT_DIR, "부서별_퇴사인원_퇴사율_결합차트.html")
