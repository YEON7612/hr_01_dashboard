...
def clean_sheet(ws, sheet_name, parse_failures):
    ym = SHEET_TO_YM[sheet_name]
    rows, subtotal_rows = [], []
    current_dept = None
...
def main():
    ...
    for sheet_name in SHEET_TO_YM:
        if sheet_name not in wb.sheetnames:
            print(f"⚠️ 시트 없음: {sheet_name} (건너뜀)")
            continue
        ws = wb[sheet_name]
        df, subtotal_df = clean_sheet(ws, sheet_name, parse_failures)
        all_rows.append(df)
    ...
    print("\n규칙 2 — 년월 컬럼 검증")
    ym_ok = cleaned["년월"].isin(SHEET_TO_YM.values()).all() and cleaned["년월"].notna().all()
    print(f"  년월 값 전부 유효({sorted(SHEET_TO_YM.values())}) 및 결측 없음: {'✅' if ym_ok else '❌'}")
